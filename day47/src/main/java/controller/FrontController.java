...
// 위의 어노테이션 덕분에 특정 Client(브라우저, 사용자)의 요청에 대해 반응할 수 있다.
// 	: 필터, 리스너(서블릿을 상속받은 특수한 서블릿)에서 사용한 어노테이션(@)
public class FrontController extends HttpServlet {
	private static final long serialVersionUID = 1L; // 직렬화
       
    public FrontController() { // 웹에서는 기본 생성자를 기본으로 활용하기 때문에 꼭 필요함!!
        super();
    }

    private ActionForward doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	// 여기에 기능을 코딩
    	...
    	// 요청 처리가 완료되면 사용자에게 응답해야 함. [VIEW로 이동]
    	
    	if(forward.isRedirect()) { // 데이터 보낼 것이 없니 ?
